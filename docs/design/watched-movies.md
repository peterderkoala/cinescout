...
card the Watched Films section uses (compact-row variant).

**Current state**: `WatchedMovies.razor` lists **every film ever crawled** — not just watched ones
— in a single flat table, each row just a title and a Watch/Unwatch toggle button. No performance
...
Model="this" FormName="watched-movies">` with two differently-named submit buttons
(`watchFilmId`/`unwatchFilmId`) per root `CLAUDE.md` — that mechanism is unchanged by this doc; only
the visual/content structure around it changes.

## Two sections

### 1. Your Watched Films

The primary section — every film currently on the active Watched list. For each:

- Film title as a section sub-heading, with a single **film-level Unwatch action** next to it (not
  per-performance — unwatching removes the whole film from the list, regardless of how many
  performances it has).
- **All** of that film's upcoming performances below the heading, each as its own
  ...
  This is the dedicated management page for watched films, with more room than Home's abbreviated
...
- **Zero-performances state**: a film can be watched with no upcoming performances at all (its run
  finished after being marked watched — `WatchedMovie` only becomes *active* for matching while
...
The add-to-watch list — every film **with at least one upcoming performance**, films whose entire
run has ended are excluded (watching one can never produce a `Match`, so listing it here is just
clutter). Each row: film title + a `Watch` button — plain, no compact-row card here, since there's
...
- [`components/film-performance-card.md`](components/film-performance-card.md) — compact-row
  variant used throughout Your Watched Films
