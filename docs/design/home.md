...
already has the material for this — an active `Match` (a `Performance` that's satisfied a tracked
...
`Cinema`/`Room`, date/time, price, match-reason badges, and a prominent booking CTA linking straight
to Kinoheld.

**Empty state** (the common case for a personal tool that mostly waits): a calm message — "No
matches yet — you'll see them here the moment a tracked film's screening fits your preferences." —
with **no call-to-action**, since waiting is the normal state once at least one film is being
tracked.

**Exception**: if there are also **zero active `TrackedMovie`s**, that's the one empty state that's
actually the user's next action, not just "waiting is normal" — show a contextual CTA pointing at
Tracked Movies (e.g. "Start by marking a film as tracked" linking to `/tracked-movies`) instead of
the plain calm message.

### 2. Secondary panels: Tracked Movies + Recent Activity
...
- **Tracked Movies** — a short list of active `TrackedMovie`s (capped to a handful, e.g. 5), each
...
- [`components/film-performance-card.md`](components/film-performance-card.md) — the hero and
  Tracked Movies panel both consume this component; update its "known consumers" list once this
