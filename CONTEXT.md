...
CineScout tracks movie screenings at Hall of Fame Kamp-Lintfort (and, in future, other cinemas), crawls seating availability, and alerts the user when a tracked film's screening matches their preferences.

## Language

**Cinema**:
One physical cinema location CineScout crawls (e.g. "HALL OF FAME - Kino in Kamp-Lintfort"). CineScout is multi-cinema-ready even though only one is configured today. Named `Cinema`, not `Site` — an earlier draft used "Site" and a later one removed it in favor of "Cinema", then the operator reversed that removal: see [ADR 0002](docs/adr/0002-rename-site-to-cinema-and-watchedmovie-to-trackedmovie.md).
_Avoid_: Site, theater, location

**Film**:
A film listing as it appears at one specific `Cinema`, matching the source API's own scoping (`detailId` + `filmSite`) — not a cross-cinema canonical movie identity. The same movie playing at two cinemas is two `Film` rows.
...
**Room**:
A named physical auditorium at a `Cinema` (e.g. "Kino 1"–"Kino 7" at Kamp-Lintfort), sourced from Kinoheld's widget config rather than the Hall-of-Fame schedule API, which has no room concept at all.
_Avoid_: Auditorium (as the entity name — Kinoheld's own vocabulary, kept as a synonym note only), Saal, screen

**TrackedMovie**:
A `Film` the user has marked as interesting. Considered active only while the film still has upcoming performances — determined at query time, not stored as a flag. Named `TrackedMovie`, not `WatchedMovie` — "watched" reads as a film the user has already attended, but the domain means a film the user wants match alerts for. See [ADR 0002](docs/adr/0002-rename-site-to-cinema-and-watchedmovie-to-trackedmovie.md).
_Avoid_: WatchedMovie, Watchlist item, favorite film
...
**Match**:
A record that one `Performance` has satisfied a `TrackedMovie`'s active preferences at a point in time. Distinct from whether a notification was actually sent for it (see `NotificationLog`).
_Avoid_: Alert (that's the outward-facing behavior a `Match` triggers, not the record itself)

**NotificationLog**:
A record of one attempt to notify the user (e.g. via Discord). Carries a `NotificationType` (`TrackStarted`, `TrackStopped`, `RulesMatched`, `SeatAvailabilityChanged`, `NewFilmAdded`); `MatchId` is only set for `RulesMatched`/`SeatAvailabilityChanged`, since track-lifecycle and new-film notifications aren't tied to a specific `Match`.
